...
export interface ResourceEvent extends ContextEvent<{
    id: string;
}> {
    readonly type: 'resource';
}
export interface StateChangeEvent extends ContextEvent<{
    oldState: string;
    newState: string;
}> {
    readonly type: 'state_change';
}
export interface ErrorEvent extends ContextEvent<{
    message: string;
}> {
    readonly type: 'error';
    metadata: EventMetadata & {
...
export declare function isResourceEvent(event: ContextEvent): event is ResourceEvent;
export declare function isStateChangeEvent(event: ContextEvent): event is StateChangeEvent;
