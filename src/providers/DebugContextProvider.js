...
export class DebugContextProvider {
    ...
    constructor(channel) {
        ...
        this.handleBreakpointChange = (event) => {
            try {
                this.channel.appendLine(`Breakpoints changed: ${event.added.length} added, ${event.removed.length} removed`);
            }
            catch (error) {
    ...
    getBreakpoints() {
        return vscode.debug.breakpoints.map((breakpoint) => ({
            ...
            location: breakpoint instanceof vscode.SourceBreakpoint
                ? {
